...
async def triage_ticket_function(config: TriageTicketConfig, builder: Builder):
    ...
    async def _triage(complaint: str) -> str:
        ...
        prompt = (
            ...
            "category — pick the SINGLE most specific match; use 'other' ONLY as a last resort:\n"
            "  billing        — charges, double charges, refunds, invoices, pricing, subscriptions, payment.\n"
            "  media_quality  — the EXPORTED / RENDERED output: blur, compression, resolution, "
            "washed-out or wrong colors, audio out of sync or missing audio, artifacts, or any "
            "mismatch between the export and the preview.\n"
            "  account_access — login, password reset, 2FA, SSO, account locked/suspended.\n"
            "  bug            — the app/editor itself crashing, freezing, spinning, or a feature "
            "(undo, timeline, playhead, import) not working.\n"
            "  other          — only if none of the above genuinely fit.\n\n"
            "confidence: your certainty in BOTH severity and category (0..1). Be calibrated — "
            "reserve >0.85 for clear-cut cases; a vague or ambiguous ticket should score lower.\n\n"
            "developer_remediation: short INTERNAL fix steps for our engineers (never shown to the customer).\n"
            "suggested_customer_reply: a STANDALONE message to the customer. It must read correctly on "
            "its own — do NOT say 'the steps above' or refer to the developer remediation or anything "
            "the customer cannot see. If the customer should do something, state it directly here.\n\n"
