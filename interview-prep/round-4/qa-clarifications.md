...
## Segment metrics glossary (also in-app: Policy page → "How to read these metrics")

`bar` = the segment's policy `min_confidence`. Two ground truths on purpose: **accept-rate /
precision** measure the **reviewed** path (a human approved/rejected); **satisfaction-rate**
measures the **auto** path (no human → the customer judges).

| Metric | Formula | Why it matters |
|---|---|---|
| `total` | count(tickets in segment) | Volume — is there enough signal to trust any rate? |
| `reviewed` | count(`decision IS NOT NULL`) | Tickets a human judged — **approve OR reject both count**. |
| `approved` | count(`decision = 'approve'`) | Of reviewed, how many the human agreed with. |
| `accept_rate` | approved / reviewed | Online accuracy on reviewed tickets. **Auto excluded** (no decision). |
| `reviewed_eligible` | reviewed tickets with `confidence ≥ bar` | Size of the confident slice we'd actually auto-act on. |
| `precision_eligible` | approved / reviewed, **within** `confidence ≥ bar` | Accuracy on the slice that would auto — **THE gate** for granting auto. |
| `feedback` | count(`customer_satisfied IS NOT NULL`) | How many CSAT responses arrived (auto-path signal volume). |
| `satisfaction_rate` | 👍 / feedback | **Auto-mode ground truth** — auto tickets are never reviewed. |
| `eligible_for_auto` | `reviewed_eligible ≥ 3 AND accept_rate ≥ 0.66 AND precision_eligible ≥ 0.66` | Promotion readiness (demo thresholds; **prod ≈ 20 / 0.95 / 0.97**). |
| `ECE` (calibration) | Σ(n·\|avg_confidence − accuracy\|) / Σn over 10 confidence bins | Is the model's confidence honest? **Lower = better calibrated** → where to set the bar. |

**The loop (closing line):** metrics **surface** a segment → readiness says if auto is **earned**
→ a human **grants** it (Promote / Add segment) → code **enforces** the ceiling.

**Views:** `segment_metrics` (per-segment) → `promotion_readiness` (+ `eligible_for_auto`) →
`calibration_bins` / `calibration_ece` (reliability). All in `schema.sql`.
